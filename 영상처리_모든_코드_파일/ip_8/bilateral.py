...
def my_bilateral(src, msize, sigma, sigma_r, pos_x, pos_y, pad_type='zero'):
    ...
    for i in range(h):
        ...
        for j in range(w):
            ...
            mask_presently = img_pad[i:i+msize,j:j+msize]  # 현재의 mask를 계속 저장

            mask=np.exp(-((x**2  + y**2)/(2 * sigma**2)))
            mask = mask*np.exp(-((midvalue-mask_presently)**2)/(2 * sigma_r * sigma_r))
            mask /= np.sum(mask)
...
if __name__ == '__main__':
    ...
    dst = my_bilateral(src_noise, 5, 100, 0.1, pos_x, pos_y)
